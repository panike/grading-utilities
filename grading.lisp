...
(defun accum-hw-poss (gr)
  (accum gr grade-record-num-hw grade-record-hw nil t t 2))

(defun accum-hw-earned (gr)
  (accum gr grade-record-num-hw grade-record-hw t nil t 2))

(defun accum-quiz-poss (gr)
  (accum gr grade-record-num-quizzes grade-record-quiz nil t t 2))

(defun accum-quiz-earned (gr)
  (accum gr grade-record-num-quizzes grade-record-quiz t nil t 2))
...
(defun accum-lec-quiz-poss (gr)
  (accum gr grade-record-num-lec-quizzes grade-record-lec-quizzes nil t t 5))

(defun accum-lec-quiz-earned (gr)
  (accum gr grade-record-num-lec-quizzes grade-record-lec-quizzes t nil t 5))

(defun accum-group-work-poss (gr)
  (accum gr grade-record-num-group-work grade-record-group-work nil t t 2))

(defun accum-group-work-earned (gr)
  (accum gr grade-record-num-group-work grade-record-group-work t nil t 2))
...
(defun accum-section-scores (gr)
  (ceiling (+ (* 40 (accum-quiz gr))
	      (* 20 (accum-hw gr))
	      (* 40 (accum-group-work gr)))))
...
(defun insert-class-participation (gr)
  (do ((i 0 (+ i 1)))
	...
	(let ((zx (if (= 0 (accum-hw-poss gr))
		      0
		      (/ x (accum-hw-poss gr) 1/2)))
	      (zy (if (= 0 (accum-quiz-poss gr))
		      0
		      (/ y (accum-quiz-poss gr) 1/2)))
	      (zz (if (>= (grade-record-att-earned gr)
			  (grade-record-att-poss gr))
		      0
		      (/ 1 (grade-record-att-poss gr)))))
	  (if (and (> zz zx)
		   (> zz zy))
	      (setf (grade-record-att-earned gr)
		    (+ 1 (grade-record-att-earned gr)))
	      (if (> zx zy)
		  (setf (car (aref (grade-record-hw gr) ix))
			(- (cdr (aref (grade-record-hw gr) ix))))
		  (setf (car (aref (grade-record-quiz gr) iy))
			(- (cdr (aref (grade-record-quiz gr) iy))))))))))
...
(defun output-personal-text (gr &optional (s *standard-output*))
  ...
  (format s "{} indicates grade will be dropped ")
  (format s "at the end of the semester~%~%")
  ;; (insert-class-participation gr)
  (grade-accum gr)
  (dotimes (i (grade-record-num-hw gr))
